...
# ------------------------------------------------------------------------------------------------ #
# Parameterise
my_ouput <- here('figures', 'supplementary', 'figure-x-busco-genomes.png')
my_width <- 30
my_height <- 15
my_unit <- "cm"
...
# Legend
my_title <- expression(paste("BUSCO assessment: Genome assemblies"))
...
# Load files
files <- dir_ls(
  path = here('data','busco','genomes','standard'),
  glob = '*.txt',
  recurse = TRUE
) %>%
  as.character()
...
# Clean names
names(files) <- c('Hydrophis curtus (AG)', 'Hydrophis elegans', 'Hydrophis major', 'Hydrophis ornatus')

# Set levels
lvls <- c('Hydrophis major', 'Hydrophis ornatus', 'Hydrophis curtus (AG)', 'Hydrophis elegans')
...
dat <- files %>%
  ...
  mutate(
    ...
    # assembly = case_when(
    #   assembly == 'hydrophis_major' ~ 'Hydrophis major',
    #   assembly == 'hydrophis_ornatus' ~ 'Hydrophis ornatus',
    #   assembly == 'hydrophis_curtus' ~ 'Hydrophis curtus (AG)',
