# ------------------------------------------------------------------------------------------------ #
# Plot BUSCO: Hydrophis curtus (West)
#
...
# ------------------------------------------------------------------------------------------------ #
# Parameterise
my_ouput <- here('figures', 'supplementary', 'figure-x-busco-hydrophis-curtus-west-genome.pdf')
my_width <- 30
my_height <- 15
my_unit <- "cm"
...
# Legend
my_title <- expression(paste("BUSCO assessment: ", italic("Hydrophis curtus"), ' (West) assembly progress'))
...
# Load files
files <- dir_ls(
  path = here('assembly', 'other-snakes', 'genome_assessment', 'busco', 'hydrophis_curtus-AG'),
  glob = '*.txt',
  recurse = TRUE
) %>%
  as.character()
